<template>
  <div class="poop-container">
        ...
        <div class="circle">
          <div class="circle-button" @click="isPooping = !isPooping">
            <lo-icon
              ...
              icon="start"
              size="3rem" />
            <lo-icon v-else color="currentColor" icon="stop" size="3rem" />
          </div>

          <span :class="!isPooping ? 'stop' : ''" class="circle-back-1"></span>
          <span :class="!isPooping ? 'stop' : ''" class="circle-back-2"></span>
        </div>
      </div>
    ...
    <div class="main">
      <div class="toilet" />
      <avatar-wrapper :avatar="leftUser.avatar" class="left-avatar" />
    ...
    <div class="footer">
      <p>
        距离你上次便便：<b>{{ timeDifference }}</b>
      </p>
      <p>
        上次便便时长：<b>{{ lastPoop.poopDuration }}</b>
      </p>
    </div>
  </div>
</template>
...
interface User extends UserState, UserBase {}
...
const lastPoop = ref<PoopBase>({
  userId: 0,
  poopType: 1,
  poopTime: '2025-02-10 00:00:00',
  poopDuration: '10分2秒',
});
...
const calculateTimeDifference = () => {
  ...
  if (!lastPoop.value.poopTime) {
    timeDifference.value = 'N/A'; // 或者显示其他提示信息
    return;
  }

  // 将 lastPoopTime 字符串转换为 Date 对象
  const lastPoopTime = new Date(lastPoop.value.poopTime);
  ...
  else if (hours > 0) timeString = `${hours}小时`;
  else if (minutes > 0) timeString = `${minutes}分钟`;
  else if (seconds > 0) timeString = `${seconds}秒`;
  ...
  .header {
    ...
    .header-left {
      .title {
        ...
        text-shadow: 0.2rem 0.2rem 0.5rem rgba(0, 0, 0, 0.2); /* 添加文字阴影 */
      }
    }
